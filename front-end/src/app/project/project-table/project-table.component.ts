...
export class ProjectTableComponent implements OnInit {
	...
	async getUserInterests() {
		const username = (await this.sessionStorageService.getUser()).username as string;

		this.interestsService.getInterestsByUsername(username).subscribe((interests) => {
			this.mappedProjects.forEach((project) => {
				const mappedProject = interests.find((interest) => project.positionId === interest.positionId);
				if (mappedProject) {
	...
	private getSeniorities() {
		this.seniorityService.getSeniorities().subscribe((seniorities) => (this.seniorities = seniorities));
	}
	...
	private getProjects() {
		return this.projectService.getProjects().subscribe((data) => {
			data.sort((prevProject, curProject) => prevProject.creationDate.valueOf() - curProject.creationDate.valueOf());
	...
	applySeniorityFilter() {
		if (this.selectedSeniorities.length === 0) {
			...
			this.dataSource.data = this.mappedProjects.filter((project) =>
				this.selectedSeniorities.some(
					(seniority) => seniority.toUpperCase().replaceAll("_", " ") === project.seniorityName,
				),
			);
		}
	}
	...
	applyUnitFilter() {
		if (this.selectedUnits.length === 0) {
			...
			this.dataSource.data = this.mappedProjects.filter((project) => this.selectedUnits.includes(project.unitName));
		}
	}
	...
	applyRoleFilter() {
		if (this.selectedRoles.length === 0) {
			...
			this.dataSource.data = this.mappedProjects.filter((project) =>
				this.selectedRoles.some((role) => role.toUpperCase().replaceAll(" ", "_") === project.roleName),
			);
		}
	}
