...
export class SettingsPreferencesComponent implements OnInit {
	...
	constructor(private readonly seniorityService: SeniorityService,
		...
		private readonly preferenceService: PreferenceService, private readonly formBuilder: FormBuilder,
		private readonly snackBar: MatSnackBar, private readonly sessionStorageService: SessionStorageService,
	) {
	...
	async ngOnInit() {
		...
		this.preferences.username = this.user.username;
	...
	private getPreferences() {
		this.preferenceService.getPreferencesForUser(this.user?.username ?? '').pipe(catchError(() => of(null)))
			.subscribe(data => {
				if (data === null) {
					this.selectedUnits = [];
					...
					this.selectedRoles =
						data.preferences.roles === this.roles ? [] : data.preferences.roles;
					this.selectedUnits =
						data.preferences.units === this.units ? [] : data.preferences.units;
				}
			})
	}
	...
	savePreferences() {
		...
		this.preferenceService.updatePreference(this.preferences, this.preferences.username).pipe(catchError(() => {
			this.snackBar.open(`Couldn't save preferences`, 'Ok', { duration: 2000 })
			return of(null);
		}))
			.subscribe(() =>
				this.snackBar.open('Preferences saved successfully', 'Ok', { duration: 2000 }));
	}
}
